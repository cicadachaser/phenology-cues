...
res=foreach(i.sim = 1:numsims, .packages=c("timeDate","zoo","vegan","scatterplot3d","dplyr")) %dopar% {
  ...
  objs=(ls(all.names=TRUE))
  list.all=mget(objs[objs != "list.all"]) #to avoid including last list.all in current list.all
