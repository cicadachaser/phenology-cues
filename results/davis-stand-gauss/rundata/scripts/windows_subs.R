...
mutation<-function(pop, sds, mutrate, N, fattail){
  ...
  #
  poptraits=select(pop, starts_with("b."))
...
runSim<-function(startpop, years.list, years.ind, N, duration, sds, mutrate, generations, traits, fattail, graphics = FALSE){
  ...
  for(g in 2:generations){
    ...
    newpop<-mutation(pop = newpop, sds = sds, mutrate = mutrate, N = N, fattail = fattail)
...
simrunner = function(N,traits,start,years.list,years.index,duration,sds,mutrate,numyears,fattail){
  b.day=b.temp=b.precip=b.moist=b.moistsq=b.cutemp=b.cuprecip=b.daysq=b.tempsq=b.precipsq=b.cutempsq=b.cuprecipsq=rep(0,N)
  ...
  newpop<-data.frame(b.day,b.temp,b.moist,b.precip,b.cutemp,b.cuprecip,b.daysq,b.tempsq,b.moistsq,b.precipsq,b.cutempsq,b.cuprecipsq)
...
yeargen<-function(dat.file, #the climate data file to use
  ...
  for(i.year in 1:length(years.list)){
    ...
    years.list[[i.year]]$moistsq = (years.list[[i.year]]$moist)^2;
