...
start<-list(#these are used to generate the starting values of individuals. Starting values will produce
  ...
  day=c(1,maxcues$day*4),
  temp=c(5,maxcues$temp*4),
  precip=c(.01,maxcues$precip*4),
  moist=c(.01,maxcues$precip*4),
  cutemp=c(5,maxcues$cutemp*4),
#   day=c(1,maxcues$day),
...
#   cutemp=c(5,maxcues$cutemp),
  ...
  daysq=c(-2*maxcues$daysq*(length(traits)),-.1*maxcues$daysq*(length(traits))),
  tempsq=c(-2*maxcues$tempsq*(length(traits)),-.1*maxcues$tempsq*(length(traits))),
  precipsq=c(-2*maxcues$precipsq*(length(traits)),-.1*maxcues$precipsq*(length(traits))),
  moistsq=c(-2*maxcues$moistsq*(length(traits)),-.1*maxcues$moistsq*(length(traits))),
  cutempsq=c(-2*maxcues$cutempsq*(length(traits)),-.1*maxcues$cutempsq*(length(traits))),
