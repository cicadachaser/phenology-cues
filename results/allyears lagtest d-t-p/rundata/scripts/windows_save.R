...
if(follow.run==TRUE){
sink(paste("follow-data",runName,".txt",sep=""))
cat("This run followed another run with different parameter values, using the final population of that run as the first population fo this run\n")
cat("And that run was  ")
cat(runsname.old)
sink()
}
